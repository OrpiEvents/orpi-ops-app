...
2. Project Settings → API → copy the Project URL and the `anon` public key, and the `service\_role` secret key.
3. SQL Editor → New query → paste the contents of `supabase\_schema.sql` → Run.
...
* `lib/activityLog.js` writes to Supabase using the service\_role key — regular users can read the log (via RLS policy) but can't write to it directly, so the audit trail can't be tampered with from the browser.
